  ...
  if (!isConnected) {
    return (
      <div className="min-h-screen bg-zinc-950 text-zinc-100 p-6 flex flex-col items-center justify-center">
        ...
        <p className="mb-6 text-zinc-400">Connect your wallet to manage onchain delegations.</p>
        <div className="flex gap-2">
          {connectors.map(c => (
            <button key={c.id} onClick={() => connect({ connector: c })} className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded">
              Connect {c.name}
            </button>
          ))}
        </div>
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-6">
      <div className="flex justify-between items-center mb-4">
        <div>
          <h1 className="text-2xl font-bold">Delegator Agent Dashboard</h1>
          <p className="text-zinc-400 text-sm">Contract: {CONTRACT_ADDRESS}</p>
          <p className="text-zinc-400 text-sm">Network: {chain?.name} | Wallet: {address}</p>
        </div>
        <button onClick={() => disconnect()} className="bg-red-800 hover:bg-red-900 px-3 py-1 rounded text-sm">
          Disconnect
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <form onSubmit={createRoot} className="border border-zinc-800 p-4 rounded bg-zinc-900">
          ...
          <div className="grid gap-2">
            ...
            <input name="intent" placeholder="Intent hash (hex, optional)" className="px-2 py-1 bg-zinc-800 rounded text-sm" />
        ...
        <form onSubmit={createSub} className="border border-zinc-800 p-4 rounded bg-zinc-900">
          ...
          <div className="grid gap-2">
            ...
            <input name="subIntent" placeholder="Intent hash (hex, optional)" className="px-2 py-1 bg-zinc-800 rounded text-sm" />
      ...
      <div>
        <div className="flex items-center gap-4 mb-2">
          <h2 className="text-xl font-bold">Delegation Tree</h2>
          <button onClick={refresh} className="px-3 py-1 bg-zinc-800 rounded text-sm">Refresh</button>
        </div>
        {tree.size === 0 ? (
          <p className="text-zinc-500">No delegations found.</p>
        ) : (
